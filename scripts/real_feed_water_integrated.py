...
REAL_D435I_CAMERA_INFO_TOPIC = "/d435i/d435i/color/camera_info"
...
def _load_continuous_tracking_config() -> dict[str, Any]:
    ...
    if not 0.0 < standoff_rate <= float(values["maximum_linear_speed_mps"]):
        raise RuntimeError("maximum standoff rate must not exceed Servo linear speed")
    if not 0.0 < standoff_progress_error <= float(values["servo_replan_enter_m"]):
...
def _approach_pixel_depth_to_base(
    *,
    image_x: float,
    image_y: float,
    depth_m: float,
    camera_info: CameraInfo,
    camera_position_base_m: list[float],
    camera_orientation_base_xyzw: list[float],
) -> dict[str, Any]:
    """Reproject one approach sample with its image-time camera transform."""
    values = [image_x, image_y, depth_m, *camera_position_base_m]
    if not all(math.isfinite(float(value)) for value in values):
        raise ValueError("approach reprojection inputs must be finite")
    if depth_m <= 0.0:
        raise ValueError("approach reprojection depth must be positive")
    if camera_info.width <= 0 or camera_info.height <= 0:
        raise ValueError("approach CameraInfo dimensions are invalid")
    fx = float(camera_info.k[0])
    fy = float(camera_info.k[4])
    cx = float(camera_info.k[2])
    cy = float(camera_info.k[5])
    if not all(math.isfinite(value) and value > 0.0 for value in (fx, fy)):
        raise ValueError("approach CameraInfo focal lengths are invalid")
    if not (0.0 <= image_x < camera_info.width and 0.0 <= image_y < camera_info.height):
        raise ValueError("approach mouth pixel is outside CameraInfo dimensions")
    point_camera = [
        (float(image_x) - cx) * float(depth_m) / fx,
        (float(image_y) - cy) * float(depth_m) / fy,
        float(depth_m),
    ]
    point_base = _add(
        camera_position_base_m,
        _rotate_tool_vector(camera_orientation_base_xyzw, point_camera),
    )
    one_pixel_x_base = _rotate_tool_vector(
        camera_orientation_base_xyzw,
        [float(depth_m) / fx, 0.0, 0.0],
    )
    return {
        "position_m": point_base,
...
def _continuous_stale_hold_policy(
    ...
    *,
    ...
    status = perception_status if isinstance(perception_status, dict) else {}
    ...
    if reason == "invalid_depth" and detected_faces >= 1:
        return {
            ...
            "timeout_sec": float(depth_reacquisition_timeout_sec),
    ...
    if reason == "no_face":
        return {
            "state": "FACE_LOST_HOLD",
            "timeout_sec": float(lost_target_timeout_sec),
...
def _approach_lateral_sign_relationship(
    *,
    image_x_delta_px: float,
    previous_base_y_m: float,
    corrected_base_y_m: float,
    previous_base_y_per_horizontal_pixel_mm: float,
    base_y_per_horizontal_pixel_mm: float,
    minimum_evidence_m: float = 0.0005,
) -> dict[str, Any]:
    """Compare observed base-Y motion with signed horizontal image motion."""
    values = (
        image_x_delta_px,
        previous_base_y_m,
        corrected_base_y_m,
        previous_base_y_per_horizontal_pixel_mm,
        base_y_per_horizontal_pixel_mm,
        minimum_evidence_m,
    )
    if not all(math.isfinite(float(value)) for value in values):
        raise ValueError("lateral sign relationship inputs must be finite")
    if minimum_evidence_m < 0.0:
        raise ValueError("minimum lateral sign evidence must be non-negative")
    average_scale_m_per_px = 0.0005 * (
        float(previous_base_y_per_horizontal_pixel_mm)
        + float(base_y_per_horizontal_pixel_mm)
    )
    pixel_contribution = float(image_x_delta_px) * average_scale_m_per_px
    corrected_delta = float(corrected_base_y_m) - float(previous_base_y_m)
    sign_evidence_available = bool(
        abs(pixel_contribution) >= minimum_evidence_m
        and abs(corrected_delta) >= minimum_evidence_m
    )
    sign_consistent = bool(
        not sign_evidence_available
        or pixel_contribution * corrected_delta > 0.0
    )
    return {
        "horizontal_pixel_contribution_base_y_m": pixel_contribution,
        "corrected_base_y_delta_m": corrected_delta,
        "sign_evidence_available": sign_evidence_available,
...
class RealIntegratedFeedWater(RealDynamicObstacleAvoidancePlan):
        ...
        *,
        ...
        self._continuous_servo_controller = ContinuousServoController(
            ContinuousServoConfig(
                ...
                maximum_standoff_rate_mps=float(
                    continuous["maximum_standoff_rate_mps"]
                ),
                standoff_progress_error_m=float(
                    continuous["standoff_progress_error_m"]
                ),
                standoff_final_tolerance_m=float(
                    continuous["standoff_final_tolerance_m"]
                ),
        ...
        self._latest_d435i_camera_info: CameraInfo | None = None
        self._continuous_approach_correction_report: dict[str, Any] = {
            "active": False,
            "accepted_samples": 0,
            "rejected_samples": 0,
            "deferred_samples": 0,
        ...
        self._d435i_camera_info_subscription = self.create_subscription(
            CameraInfo,
            REAL_D435I_CAMERA_INFO_TOPIC,
            self._d435i_camera_info_callback,
            10,
        )
    ...
    def _d435i_camera_info_callback(self, message: CameraInfo) -> None:
        if message.width > 0 and message.height > 0 and message.k[0] > 0.0 and message.k[4] > 0.0:
            self._latest_d435i_camera_info = message

    def _correct_continuous_approach_payload(
        self,
        payload: dict[str, Any],
    ) -> tuple[dict[str, Any] | None, dict[str, Any]]:
        """Correct candidates only during Servo approach using image-time TF."""
        camera_info = self._latest_d435i_camera_info
        if camera_info is None:
            return None, {"accepted": False, "reason": "approach_camera_info_unavailable"}
        try:
            ...
            if source_age_sec > maximum_source_age_sec:
                raise ValueError(
                    "approach source frame is stale "
                    f"({source_age_sec:.3f}s > {maximum_source_age_sec:.3f}s)"
                )
            if source_age_sec < -0.10:
                raise ValueError(
                    f"approach source frame is {abs(source_age_sec):.3f}s in the future"
                )
            candidates = payload["candidates"]
            if not isinstance(candidates, list) or not candidates:
                raise ValueError("candidate list is empty")
            stamp_nanoseconds = int(round(source_stamp * 1_000_000_000.0))
            stamp_sec, stamp_nsec = divmod(stamp_nanoseconds, 1_000_000_000)
            stamp = Time.from_msg(
                TimeMsg(
                    sec=int(stamp_sec),
                    nanosec=int(stamp_nsec),
                )
            )
            ...
            for raw_candidate, corrected_candidate in zip(
                candidates, corrected["candidates"]
            ):
                reprojection = _approach_pixel_depth_to_base(
                    image_x=float(raw_candidate["image_x"]),
                    image_y=float(raw_candidate["image_y"]),
                    depth_m=float(raw_candidate["depth_m"]),
                    camera_info=camera_info,
                    camera_position_base_m=camera_position,
                    camera_orientation_base_xyzw=camera_orientation,
                )
                original_position = [
                    float(value) for value in raw_candidate["position"]
                ]
                corrected_candidate["position"] = [
                    round(float(value), 6) for value in reprojection["position_m"]
                ]
                sample_reports.append(
                    {
                        ...
                        "image_y": float(raw_candidate["image_y"]),
                        "depth_m": float(raw_candidate["depth_m"]),
                        "original_position_m": original_position,
                        "corrected_position_m": list(reprojection["position_m"]),
                        "correction_m": _subtract(
                            reprojection["position_m"], original_position
                        ),
                        "base_y_per_horizontal_pixel_mm": reprojection[
                            "base_y_per_horizontal_pixel_mm"
                        ],
                    }
                )
            ...
            return None, {
                "accepted": False,
                "reason": f"approach_reprojection_failed:{exc}",
            }
    ...
    def _mouth_candidates_callback(self, message: String) -> None:
        ...
        if self._continuous_approach_correction_active:
            try:
                ...
                correction = {
                    "accepted": False,
                    "reason": f"approach_payload_decode_failed:{exc}",
                }
        ...
        try:
            ...
            return
        if self._continuous_approach_correction_active:
            selected_visible = visible[selected_index]
            selected_x = float(selected_visible["image_x"])
            selected_y = float(selected_visible["image_y"])
            selected_correction = min(
                correction["samples"],
                key=lambda sample: math.hypot(
                    float(sample["image_x"]) - selected_x,
                    float(sample["image_y"]) - selected_y,
                ),
            )
            previous = self._continuous_approach_previous_selected_sample
            relationship = {
                "image_x": float(selected_correction["image_x"]),
                "image_y": float(selected_correction["image_y"]),
                "depth_m": float(selected_correction["depth_m"]),
                "base_y_per_horizontal_pixel_mm": float(
                    selected_correction["base_y_per_horizontal_pixel_mm"]
                ),
                "corrected_base_y_m": float(position[1]),
                "image_x_delta_px": None,
                "corrected_base_y_delta_m": None,
                "horizontal_pixel_contribution_base_y_m": None,
            }
            if previous is not None:
                pixel_delta = relationship["image_x"] - float(previous["image_x"])
                lateral = _approach_lateral_sign_relationship(
                    image_x_delta_px=pixel_delta,
                    previous_base_y_m=float(previous["corrected_base_y_m"]),
                    corrected_base_y_m=relationship["corrected_base_y_m"],
                    previous_base_y_per_horizontal_pixel_mm=float(
                        previous["base_y_per_horizontal_pixel_mm"]
                    ),
                    base_y_per_horizontal_pixel_mm=relationship[
                        "base_y_per_horizontal_pixel_mm"
                    ],
                )
                relationship.update(
            ...
            self._continuous_approach_correction_report[
                "last_selected_scale_relationship"
            ] = relationship
            if previous is not None and not relationship["sign_consistent"]:
                self._continuous_approach_correction_report[
                    "lateral_sign_diagnostic_mismatches"
                ] = int(
                    self._continuous_approach_correction_report.get(
                        "lateral_sign_diagnostic_mismatches", 0
                    )
                ) + 1
            # The wrist camera moves with Servo, so horizontal image motion is
            # not an independent base-Y command and may legitimately oppose
            # the timestamped absolute 3D delta. Keep this comparison for
            # diagnosis only; every fresh valid reprojected XYZ observation
            # must reach the Cartesian tracker.
            relationship["lateral_update_mode"] = (
                "timestamped_absolute_3d_pixel_sign_diagnostic_only"
            )
            self._continuous_approach_previous_selected_sample = relationship
    ...
    def _publish_continuous_diagnostics(
        ...
        *,
        ...
        commanded_standoff_m: float | None = None,
        ...
        report.update(
            {
                ...
                "commanded_standoff_m": commanded_standoff_m,
    ...
    def _continuous_servo_approach_and_hold(
        ...
        *,
            ...
            while rclpy.ok():
                ...
                if not target.available and target.reason in {
                    ...
                    hold_policy = _continuous_stale_hold_policy(
                        ...
                        depth_reacquisition_timeout_sec=float(
                            self._continuous_parameters[
                                "depth_reacquisition_timeout_sec"
                            ]
                        ),
                    )
                    hold_state = str(hold_policy["state"])
                    hold_timeout = float(hold_policy["timeout_sec"])
                    timeout_reason = str(hold_policy["timeout_reason"])
                    if hold_state == "DEPTH_TEMPORARILY_INVALID":
                        report["depth_invalid_zero_velocity_cycles"] += 1
                        report["maximum_depth_invalid_age_sec"] = max(
                            float(report["maximum_depth_invalid_age_sec"]),
                    ...
                    if len(diagnostics) > 100:
                        diagnostics.pop(0)
                    if target.age_sec > hold_timeout:
                        report["stop_reason"] = timeout_reason
                        break
                    continue
                ...
                self._publish_continuous_diagnostics(
                    ...
                    commanded_standoff_m=decision.commanded_standoff_m,
                    requested_standoff_m=decision.requested_standoff_m,
                    standoff_transition_active=(
                        decision.standoff_transition_active
                    ),
                ...
                if decision.recovery_required:
                    ...
                    # processed corrected frame; provisional is sufficient.
                    # Servo remains inactive and publishes no motion here.
                    # Recovery moved the wrist camera, so begin a new lateral
                    # pixel-delta anchor instead of comparing pixels across
                    # two different camera poses.
                    self._continuous_approach_previous_selected_sample = None
                    post_recovery_acquisition = self._acquire_continuous_target()
                    recovery["post_recovery_target_acquisition"] = {
                        key: value
                        for key, value in post_recovery_acquisition.items()
                        if key != "target"
                    }
                    if not post_recovery_acquisition.get("success"):
                        report["stop_reason"] = (
                            post_recovery_acquisition.get("reason")
                            or "fresh target unavailable after continuous recovery"
                        )
                        break
        ...
        report["approach_coordinate_correction"] = copy.deepcopy(
            self._continuous_approach_correction_report
        )
