...
class InitialTargetAcquirer:
    """Wait for stability, then permit only a conservative provisional target."""
        ...
        *,
        ...
        if target.available:
            return InitialAcquisitionDecision(
                ...
                False,
                "provisional_target_after_acquisition_timeout",
            )
        return InitialAcquisitionDecision(
            True,
            target,
...
class ContinuousMouthTracker:
    ...
    def add_observation(
        ...
        *,
        ...
        if self._observations and (
            source_stamp <= self._observations[-1].source_timestamp_sec
            or received <= self._observations[-1].received_monotonic_sec
        ):
            self._last_rejection = "non_monotonic_timestamp"
            return False, self._last_rejection
        self._observations.append(
            ContinuousMouthObservation(
                position_m=point,
    ...
    def target(self, *, now_monotonic_sec: float) -> ContinuousMouthTarget:
        ...
        if not self._observations:
            return self._empty_target(self._state, self._last_rejection)
